#!/usr/bin/env bash 无缺失指示器

# ridge就是线性回归，rf就是随机森林算法，都是机器学习算法
set -euo pipefail
...
echo "[1/3] 训练 Daily_stress (no missing indicator)..."
...
echo "[2/3] 训练 STAI1 (no missing indicator)..."
python3 "$TRAIN_DIR/train.py" \
  --data-path "$ROOT_DIR/processed/mmash_user_level_features.csv" \
  --output-dir "$OUT_DIR" \
  --target label_stai1 \
...
for f in files:
    ...
    rows.append({
        ...
        'model': obj.get('model'),
        'setting': 'without_missing_indicator',
...
summary_path = out_dir / 'summary_all_tasks_ridge_without_mi.csv'
